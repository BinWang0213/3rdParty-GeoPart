...
newton_solver_params = dict(atol=1e-9, maximum_iterations=5, rtol=1e-7)
...
class BenchmarkParameters:
    ...
    Ra: dolfin.Constant = dolfin.Constant(7e5)
...
class King2010ALASolver:
    ...
    def run(self, comm, ElementClass, AdvectorClass,
        ...
        rhou = element_cls.get_velocity(U)
        ...
        def uh_accessor(step, dt):
            ...
            element_cls.solve_diffusion(
                ...
                **newton_solver_params)

            velocity_projector.project(u, u_vec)
        ...
        normal_bcy = dolfin_dg.DGDirichletNormalBC(
            ds(1), dolfin_dg.tangential_proj(rhou, n))
        normal_bcy.component = 1
        normal_bcx = dolfin_dg.DGDirichletNormalBC(
            ds(2), dolfin_dg.tangential_proj(rhou, n))
        normal_bcx.component = 0
        stokes_bcs = [normal_bcx, normal_bcy]
        ...
        f_u = rhobar * ghat * (Di * p - Ra * (T - Tref))
        f_T = phi * Di/Ra \
              + Di*rhobar*ufl.dot(ghat, u) * alpha * (T + To)
