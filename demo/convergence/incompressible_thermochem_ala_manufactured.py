...
class MMSALAThermoChemBoussinesq:
    bounded = None
    ...
    def run(self, comm, ElementClass, CompositionClass,
            AdvectorClass):
        ...
        for i, n_val in enumerate(n_vals):
            ...
            u_vec = dolfin.Function(
                dolfin.FunctionSpace(mesh, ufl.VectorElement("DG",
                                                             ufl.triangle, 1)))
            element_cls = ElementClass(ptcls, u_vec, dt_ufl, heat_property_idx)
            ...
            element_cls.T0_a.interpolate(T_soln)
            # T.interpolate(T_soln)

            # Add particles to cells in a sweep. Don't need to bound the
            # sweep as we interpolate phi afterwards.
            n_parts_per_cell = 25*element_cls.degree()
            ad = leopart.AddDelete(ptcls, n_parts_per_cell, n_parts_per_cell,
                                   [phi, Tdiff[0], element_cls.dTh0])
            ad.do_sweep()
            ptcls.interpolate(element_cls.T0_a, 2)
            ...
            def uh_accessor(step, dt):
                ...
                element_cls.project_advection(Tstar, heat_bcs)
            ...
            stokes_bcs = [dolfin_dg.DGDirichletBC(ds(1), u_soln)]
            ...
            f_expr = 3*u0/r * ufl.as_vector((x[1], -x[0])) \
                * (ufl.cos(t_ufl)**2 + 0.5)
            f_u = f_expr
            f_u += Rb * (phi - phi_D) * dolfin.Constant((0, -1))
            f_u += Ra * (T - T_soln) * dolfin.Constant((0, 1))

            f_T = dolfin.Constant(0.0)

            # Initial Stokes solve to get u(x, t=0) and p(x, t=0)
            Tstar[0].interpolate(T_soln)
            Tstar[1].interpolate(T_soln)

            element_cls.dt_ufl.assign(1e-4)
            ...
            for j in range(400):
                ...
                if n_val == 16:
                    dolfin.XDMFFile("thermochemala_phi.xdmf").write_checkpoint(
                        phi, "phi", time_step=t, append=j!=0
                    )
                    projector.project(phi_D, phi_exact)
                    dolfin.XDMFFile(
                        "thermochemala_phi_exact.xdmf").write_checkpoint(
                        phi_exact, "phi_exact", time_step=t, append=j!=0
                    )
                    dolfin.XDMFFile("thermochemala_T.xdmf").write_checkpoint(
                        element_cls.get_temperature_sub_function(U), "T",
                        time_step=t, append=j!=0
                    )
                    dolfin.XDMFFile("thermochemala_u.xdmf").write_checkpoint(
        ...
        if IS_TEST_ENV:
            self.check_rates(rates_u_l2, element_cls, rates_phi_l2,
                             composition_cls, rates_T_l2)

    def check_rates(self,
                    rates_u_l2, element_cls,
                    rates_phi_l2, composition_cls,
                    rates_T_l2):
        assert np.all(rates_u_l2 > ((element_cls.degree() + 1) - 0.25))
        assert np.all(rates_phi_l2 > ((composition_cls.degree() + 1) - 0.25))
        assert np.all(rates_T_l2 > ((element_cls.degree() + 1) - 0.25))


class MMSALAThermoChemBoussinesqNonSmooth(MMSALAThermoChemBoussinesq):
    bounded = None
...
class MMSALAThermoChemBoussinesqNonSmoothBounded(
        MMSALAThermoChemBoussinesqNonSmooth):
    bounded = (0.0, 1.0)
...
if __name__ == "__main__":
    ...
    experiment_classes = (
        MMSALAThermoChemBoussinesq,
        # MMSALAThermoChemBoussinesqNonSmooth,
        # MMSALAThermoChemBoussinesqNonSmoothBounded,
    )

    # RK3 usually gives optimal rates on the finer meshes
